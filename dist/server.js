...
const server = new McpServer({ name: "target-mcp", version: "1.0.0" }, {
    capabilities: { tools: {} },
    instructions: "MCP server for Target store data. Find stores by ZIP code, look up product details by TCIN, and search products by keyword.",
});
server.registerTool("find_stores", {
    description: "Find Target stores near a ZIP code. Returns store names, addresses, phone numbers, and store URLs.",
    inputSchema: {
        zip: z
            .string()
            .length(5)
    ...
    try {
        ...
        const lines = stores.map((s, i) => `${i + 1}. **${s.name}** (ID: ${s.store_id})\n` +
            `   ${s.address}\n` +
...
server.registerTool("lookup_product", {
    description: "Look up a Target product by its TCIN (product ID). Returns title, price, and image.",
    inputSchema: {
        tcin: z
            .string()
...
server.registerTool("search_products", {
    description: "Search Target products by keyword. Returns product titles, TCINs, prices, images, brands, ratings, shipping stock status, and per-store pickup/in-store availability at a specific store. REQUIRES store_id and zip from find_stores results. Uses a browser to bypass anti-bot protections (~10s).",
    inputSchema: {
    ...
    try {
        ...
        const lines = results.map((r, i) => {
            const storeLines = r.available_stores.map((s) => `     - ${s.store_name} (${s.store_id}): ` +
