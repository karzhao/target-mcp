...
async function runConcurrent<T>(
  items: T[],
  fn: (item: T) => Promise<StoreSummary>,
  concurrency: number
...
export async function findStoresByZip(
  zip: string
): Promise<StoreSummary[]> {
...
export async function lookupProduct(tcin: string): Promise<ProductInfo> {
...
async function zipToCoords(zip: string): Promise<{ lat: string; lng: string } | null> {
  try {
...
export async function searchProducts(
  ...
  try {
    ...
    const products =
      slpData.data_source_modules?.[0]?.module_data?.search_response
        ?.products || [];
    ...
    return products.slice(0, limit).map((p: any) => {
      ...
      const availableStores: StoreAvailability[] = storeOpts.map((so: any) => {
